...
void LV::compute_rhs() {
  ...
  const double alpha_robin = cardiac::constants::guccione::ALPHA_ROBIN;
    ...
    auto local_rhs_assembler =
      [&](Tensor<1, expected_dofs_per_cell, VecADNumberType> &local_u) {
        ...
        for (unsigned int q = 0; q < n_q; ++q) {
          ...
          Tensor<2, dim> P;
          Tensor<4, dim> dP_dF;
          t_utils.compute_tensors(F_d, fe_values.quadrature_point(q), P, dP_dF);
                               ...
                               const std::string &csv_filename)
{
  ...
  for (const auto &mesh_file : mesh_files) {
    ...
    model->setup();
    model->solve(cardiac::constants::guccione::CONVERGENCE_STUDY_STEPS);
